...
import { PlusCircle } from "lucide-react"; // Lucide React icon
import EditorialsSection from "@/components/editorials-section";
import GuestOpinionsSection from "@/components/guest-opinions-section";
import VideosSection from "@/components/videos-section";
import LettersToTheEditor from "@/components/letters-to-the-editor";
import CartoonsSection from "@/components/cartoons-section";
import NewestColumns from "@/components/newest-columns";
import { Header } from "@/components/wp/header";
import { Footer } from "@/components/wp/footer";
import { AmazonAd, MacbookAd, SpotifyAd } from "@/components/adds-section";
    ...
    opinions: {
        list: [
            ...
            },
            {
                ...
                authorInitials: "KP",
                title: "Alex Murdaugh’s murder appeal could hang on a silenced juror",
                date: "February 14, 2026",
            },
        ],
    },
...
const tabs = [
    ...
    "Submitt a Guest Opinion",
    "Submit a Letter"
];

export default function OpiniosPage() {
    const { featured, secondary, moreStories, sidebar, opinions } = pageData;
                ...
                <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-8 sm:py-12">
                    ...
                    <div className="mb-6 border-b">
                        <div className="flex items-center gap-3 mb-2">
                            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-bold tracking-tight">Opinions</h1>
                            <PlusCircle className="w-4 h-4 sm:w-5 sm:h-5" />
                        </div>

                        {/* CATEGORY TABS */}
                        <div className="flex flex-wrap gap-2 sm:gap-6 mb-2">
                            {tabs.map((tab) => (
                                <button
                                    key={tab}
                                    className="font-sans flex items-center gap-3 py-1 text-xs sm:text-sm lg:text-base hover:bg-gray-100 hover:rounded-xl transition"
                                >
                                    {tab}
                                </button>
                            ))}
                        ...
                        <>
                            <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-start mb-12">
                                <div className="max-w-[520px]">
                                    <h2 className="text-xl sm:text-2xl font-serif font-bold leading-[1.1]">{featured.title}</h2>
                                    <p className="mt-4 sm:mt-6 text-sm sm:text-base lg:text-lg text-gray-700">{featured.description}</p>
                                    <p className="mt-4 sm:mt-6 text-xs sm:text-sm text-gray-500">{featured.date}</p>
                                </div>
                            ...
                            {/* SECONDARY STORIES */}
                            <section className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 border-t pt-8 sm:pt-10 mb-12">
                                {secondary.map((item, index) => (
                                    <div
                                        key={index}
                                        className={`grid grid-cols-[1fr_100px] sm:grid-cols-[1fr_140px] lg:grid-cols-[1fr_200px] gap-4 sm:gap-6 ${index === 1 ? "md:border-l md:pl-8" : ""
                                            }`}
                                    >
                                        <div>
                                            <h3 className="text-base sm:text-lg lg:text-xl font-bold">{item.title}</h3>
                                            <p className="text-xs sm:text-sm text-gray-500 mt-2">{item.date}</p>
                                        </div>
                                        <Image src={item.image} alt={item.title} width={400} height={300} className="w-full h-auto object-cover" />
                                    </div>
                                ))}
                            </section>
                        </>
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 lg:gap-14 pt-5">
                        <div className="lg:col-span-2">
                            {currentPage === 1 && (
                                <h2 className="text-xl sm:text-2xl font-serif font-bold lg:mb-0 lg:mt-5 sm:mb-10 pb-2 pt-2">
                                    More Stories
                                </h2>
                            )}
                            ...
                            <div className="space-y-8 sm:space-y-10 lg:space-y-14">
                                {paginatedStories.map((story, index) => (
                                    <div
                                        key={index}
                                        className="grid grid-cols-[1fr_100px] sm:grid-cols-[1fr_140px] lg:grid-cols-[1fr_260px] gap-4 sm:gap-6 lg:gap-8"
                                    >
                                        <div>
                                            <h3 className="text-sm sm:text-base lg:text-lg font-serif font-bold cursor-pointer">{story.title}</h3>
                                        ...
                                        <Image
                                            ...
                                            width={500}
                                            height={350}
                                            className="w-[100px] sm:w-[140px] lg:w-[260px] h-[70px] sm:h-[100px] lg:h-[170px] object-cover"
                                        />
                                    </div>
                                ))}
                            </div>
                        ...
                        <aside className="space-y-8 sm:space-y-10">
                            <div className="border p-4 sm:p-6 text-center">
                                <h3 className="font-semibold text-sm sm:text-base lg:text-lg">{sidebar.newsletter.title}</h3>
                                <p className="text-xs sm:text-sm text-gray-600 mt-2">{sidebar.newsletter.description}</p>
                                <button className="mt-4 bg-black text-white px-4 sm:px-6 py-2 text-xs sm:text-sm hover:bg-gray-800 transition">
                                    {sidebar.newsletter.buttonText}
                                </button>
                            </div>

                            {/* Realistic Dummy Ad with Image */}
                            <div className="relative bg-white border shadow-sm overflow-hidden h-[250px] sm:h-[350px] lg:h-[600px] flex flex-col">
                                {/* Sponsored Label */}
                                ...
                                {/* Dummy Ad Image */}
                                <img src="https://picsum.photos/600/800" alt="Ad" className="h-1/2 w-full object-cover" />

                                {/* Ad Content */}
                                <div className="flex flex-col justify-between flex-1 p-4 sm:p-6">
                                    <div>
                                        <h4 className="text-base sm:text-lg lg:text-xl font-semibold">Upgrade Your Workspace</h4>
                                        <p className="text-xs sm:text-sm text-gray-600 mt-2">
                                            Premium tools designed to improve focus, comfort, and productivity.
                                        </p>
                                    </div>

                                    <button className="mt-4 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-xs sm:text-sm rounded transition">
                                        Shop Now
                                    </button>
                                </div>
                            </div>
